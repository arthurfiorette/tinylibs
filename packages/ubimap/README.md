...
```js
const { deferred } = require('ubimap');
import { deferred } from 'ubimap';
```
...
```ts
import { deferred } from 'https://cdn.skypack.dev/ubimap@latest';
```
...
```js
import { deferred, isDeferred } from 'ubimap';
